...
// is free or is located anywhere below a free room in the same column is not considered suitable 
...
function matrixElementsSum(matrix) {
    ...
    for (let i = 0; i < matrix.length; i++) {
        for (let j = 0; j < matrix[i].length; j++) {
            ...
            else if (invalidIndexes.indexOf(j) === -1) {
                total += matrix[i][j];
            }
        }
    }
...
console.log(matrixElementsSum(test1));  // answer = 9
...
console.log(matrixElementsSum(test2));  // answer = 9
...
console.log(matrixElementsSum(test3));  // answer = 18

const test4 = [[0]]
console.log(matrixElementsSum(test4));  // answer = 0
